 ...
 * Completed by Ruiyang Chang (ruiyangc) in Feb 1 2021
 * 
...
function largest(lst) {
  let largest= lst[0]; // default as the first one num in the array
  for (i = 0; i <= lst.length; i++){ // go throught all num in the array
      if (lst[i] > largest) {
          largest = lst[i]; // if this num is bigger than the current, replace the largest
      }
  }
  return largest;
}
...
function fact(n) {
  // use recursion here
  if (n == 0){ // in the very end it recurse to 0 ,and return 1
    return 1;
  }
  return (n * fact(n - 1)); // recurses several times
}
...
function fib(n) {
  // again, use recursion
  if (n == 0){ // in the end, goes to 0...
    return 0;
  } else if (n == 1){
    return 1; // and 1.
  }
  return (fib(n - 1) + fib(n - 2)); // recurses several times
}